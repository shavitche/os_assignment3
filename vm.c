...
void swap_out_page(struct proc* p){
  ...
  if (index_of_swapped_out_page != -1) {
    ...
    // write to swapfile
    writeToSwapFile(p, (char *) p->page_descriptions[index_of_swapped_out_page].virtual_address,
                    (uint) p->next_free_place_in_swapfile, PGSIZE);
    cprintf("Saved to file va: %x in %d\n", p->page_descriptions[index_of_swapped_out_page].virtual_address,
            p->next_free_place_in_swapfile);
    ...
    *pte_of_swapped_out_page = *pte_of_swapped_out_page | PTE_PG;
    ...
    // update new place in swapfile
    update_next_free_place_in_swapfile(p);
    // cprintf("Finished swaping out %x\n", p->page_descriptions[index_of_swapped_out_page].virtual_address);
  }
}
...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  for(; a < newsz; a += PGSIZE){
    ...
    // Check current page memory status, if we have too much pages, swap something out
    ...
    if(number_of_pages_in_memory == MAX_PSYC_PAGES && proc->pid > 2){
      if(number_of_pages_in_swapfile > MAX_PSYC_PAGES){
        panic("To many pages!\n");
      }
      // cprintf("Calling swap\n");
      swap_out_page(proc);
    }
#endif
...
//  }
//  return newsz;
    //TODO: Check if this change break anything
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);

#ifndef NONE
    if(proc->pid >2){
      // enter new page to our struct
      uint* b = (uint*)PGROUNDDOWN(a);
      pte_t* pte = walkpgdir(pgdir,b,0);
      if(insert_page_description(proc, pte, PGROUNDDOWN(a)) == 1){
...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if(!(*pte & PTE_P) && (*pte & PTE_PG)){
      ...
      // Page not in memory but is in swapfile, create pte for it.
...
pgflt_handler(struct proc* p, uint va_fault){
  ...
  cprintf("handling fault: %x\n", PGROUNDDOWN(va_fault));
  p->page_fault_counter++;
  int found = 0;
  pte_t *pte_of_incoming_page = walkpgdir(p->pgdir, (char*)PGROUNDDOWN(va_fault), 0);
  if(!(*pte_of_incoming_page & PTE_PG)){
    panic("Real seg fault :oooo\n");
  }

  int index_of_incoming_page;
  for(index_of_incoming_page = 0; index_of_incoming_page < MAX_TOTAL_PAGES; index_of_incoming_page++){
    if(p->page_descriptions[index_of_incoming_page].status == PG_SWAPFILE &&
       PGROUNDDOWN(p->page_descriptions[index_of_incoming_page].virtual_address) == PGROUNDDOWN(va_fault)){
      found = 1;
      break;
    }
  }
  ...
  if(found == 0){
    panic("Didn't find the faulting page!");
  }

  // Got the right index
  // check if we need to swap out a page:
  if(get_number_of_pages_in_memory(p) == MAX_PSYC_PAGES && proc->pid > 2){
    swap_out_page(p);
  }

  // swap in the faulting page
  char* mem = kalloc();
  uint read_from_offset = (uint)p->page_descriptions[index_of_incoming_page].swap_file_offset;
  readFromSwapFile(p, mem, read_from_offset, PGSIZE);
  mappages(p->pgdir, (char *)PGROUNDDOWN(p->page_descriptions[index_of_incoming_page].virtual_address),
           PGSIZE, v2p(mem), PTE_W|PTE_U|PTE_P);
  ...
  *pte_of_incoming_page = *pte_of_incoming_page & ~PTE_PG;
  ...
  update_next_free_place_in_swapfile(p);
  insert_page_to_linked_list(p, index_of_incoming_page);
}
...
choose_by_LIFO(struct proc* p){
  ...
  // Pick the page that was inserted last to the memory
  // proc->head is the newest page to be inserted into memory.
  // But we need to make sure that the page we want to page out is a user page
  swaped_out_page_description = p->head;
  while (1) {
    if(swaped_out_page_description->status == PG_MUST_MEM){
      swaped_out_page_description = swaped_out_page_description->next;
      continue;
    }
    pte_to_swap_out = walkpgdir(p->pgdir, (void *) swaped_out_page_description->virtual_address, 0);

    // find a page that is present to page out
    if ((*pte_to_swap_out & PTE_P) != 0) {
      break;
    }
    swaped_out_page_description = swaped_out_page_description->next;
  }
  ...
  for(i = 0; i < MAX_TOTAL_PAGES; i++){
    if(p->page_descriptions[i].status == PG_MEMORY &&
       p->page_descriptions[i].virtual_address == swaped_out_page_description->virtual_address){
      break;
    }
  }
  return i;
...
int choose_by_SCFIFO(struct proc* p){
  struct proc *proc = myproc();
  ...
  while (1) {
    ...
    if(swaped_out_page_description->status == PG_MEMORY){
      ...
      // if the page access bit is on
      if ((*pte_to_swap_out & PTE_A) != 0) {

        *pte_to_swap_out &= ~PTE_A;
      } else {
        break;
      }
    }
...
get_page_lap(struct proc *p){
  ...
  for(i = 0; i < MAX_TOTAL_PAGES; i++){
    if(p->page_descriptions[i].status == PG_MEMORY){
      ...
      if(least_accessed == -1 || p->page_descriptions[i].access_counter < least_accessed){
...
void lap_update(){
  ...
  if (proc) {
    ...
    for (i = 0; i < MAX_TOTAL_PAGES; i++){
      if(proc->page_descriptions[i].status == PG_MEMORY || proc->page_descriptions[i].status == PG_MUST_MEM){
